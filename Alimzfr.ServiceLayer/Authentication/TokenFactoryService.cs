...
namespace Alimzfr.ServiceLayer.Authentication
{
    ...
    public class TokenFactoryService : ITokenFactoryService
    {
        ...
        {
            var (accessToken, claims, accessTokenExpireDate) = await createAccessTokenAsync(user);
            var (refreshTokenValue, refreshTokenSerial, refreshTokenExpireDate) = createRefreshToken();
            return new JwtTokensData
            {
                ...
                AccessTokenExpirationDate = accessTokenExpireDate,
                RefreshTokenExpirationDate = refreshTokenExpireDate,
                Claims = claims
            };
        }

        private (string RefreshTokenValue, string RefreshTokenSerial, DateTime RefreshTokenExpireDate) createRefreshToken()
        {
            ...
            var refreshTokenExpireDate = now.AddMinutes(_configuration.Value.RefreshTokenExpirationMinutes);
            ...
            return (refreshTokenValue, refreshTokenSerial, refreshTokenExpireDate);
        }
        ...
        private async Task<(string AccessToken, IEnumerable<Claim> Claims, DateTime AccessTokenExpireDate)> createAccessTokenAsync(User user)
        {
            var claims = new List<Claim>
            {
                // Unique Id for all Jwt tokes
            ...
            var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
            var now = DateTime.UtcNow;
            var accessTokenExpireDate = now.AddMinutes(_configuration.Value.AccessTokenExpirationMinutes);
            var token = new JwtSecurityToken(
                issuer: _configuration.Value.Issuer,
                audience: _configuration.Value.Audience,
                claims: claims,
                ...
                signingCredentials: creds);
            return (new JwtSecurityTokenHandler().WriteToken(token), claims, accessTokenExpireDate);
        }
    }
}